...
SELECT CONCAT(r.surname,' ',r.firstname) AS member, CONCAT(m.surname,' ',m.firstname) AS recommended_by
FROM Members m JOIN Members r
...
SELECT f.name AS Facility, MONTH(b.starttime) AS month, COUNT(b.memid) AS Member_usage
FROM Bookings AS b
INNER JOIN Facilities AS f
ON f.facid = b.facid
WHERE b.memid <> 0
